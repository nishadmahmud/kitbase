                ...
                {tools.map((tool) => {
                    ...
                    return (
                        <Link
                            key={tool.href}
                            href={tool.href}
                            className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-7 no-underline transition-all hover:bg-gray-50 dark:hover:bg-gray-800 hover:border-gray-300 dark:hover:border-gray-700 hover:shadow-md hover:shadow-gray-200/50 dark:hover:shadow-black/20 group"
                        >
                            <div className="w-12 h-12 rounded-xl bg-gray-50 dark:bg-gray-800 border border-gray-100 dark:border-gray-700 flex items-center justify-center mb-4 group-hover:bg-blue-50 dark:group-hover:bg-gray-700 group-hover:border-blue-100 dark:group-hover:border-gray-600 transition-colors">
                                <ToolIcon className="w-6 h-6 text-gray-600 dark:text-gray-200 group-hover:text-blue-600 dark:group-hover:text-white transition-colors" />
                            </div>
                            <h3 className="font-semibold text-gray-900 dark:text-gray-200 m-0 mb-2 text-base group-hover:text-blue-700 dark:group-hover:text-blue-400 transition-colors">{tool.name}</h3>
                            <p className="text-sm text-gray-500 dark:text-gray-400 leading-relaxed m-0 group-hover:text-gray-600 dark:group-hover:text-gray-300 transition-colors">{tool.description}</p>
                        </Link>
                    );
                })}
            </div>
